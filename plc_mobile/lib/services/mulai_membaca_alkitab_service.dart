  ...
  try {
    ...
    final response = await http
        .post(
          ...
          body: json.encode({'start_date': tanggal}),
        )
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint(
        'Has tanggal_mulai field: ${jsonMap.containsKey('tanggal_mulai')}',
      );
...
Future<MembacaAlkitabResponse?> resetMembacaAlkitab({
  required String token,
  required String tanggal,
}) async {
  try {
    // Endpoint untuk reset start date menggunakan PUT method
    final raw = '/api/reading/start-date';
    final uri = buildApiUri(raw);

    debugPrint('Calling API (PUT): $uri');
    debugPrint('Token: ${token.substring(0, 20)}...');
    debugPrint('Tanggal Reset: $tanggal');
    ...
    debugPrint('Error resetting membaca Alkitab: $e');
    rethrow;
  }
}
